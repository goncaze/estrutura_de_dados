...
class App:
    def __init__(self, page: ft.Page):
        ...
        # self.row_lista = ft.Row(controls=[self.noh], expand=True)
        self.row_lista = ft.Row(
            expand=True, auto_scroll=True, scroll=ft.ScrollMode.ALWAYS
        )

        self.ctn_lista = ft.Container(
            content=self.row_lista,
            expand=True,
        ...
        self.txt_comprimento = ft.Text(
            value=f"Comprimento da lista: {len(self.row_lista.controls)}", size=19
        )
        ...
        self.btn_inserir_final = ft.CupertinoButton(
            ...
            on_click=self.clk_inserir_final_d,
            expand=True,
            border_radius=5,
            color=ft.colors.WHITE,
            bgcolor="#d5085c",
    ...
    def criar_noh(self, valor) -> ft.Container:
        ctn_novo_noh = ft.Container(
            border=ft.border.all(width=3, color=ft.colors.BLUE_800),
            padding=0,
            margin=0,
                    ...
                    ft.Container(
                        ...
                        border=ft.border.only(
                            right=ft.border.BorderSide(
                                width=1, color=ft.colors.BLUE_800
                            )
                        ),
                    ),
                    ft.Container(
                        ...
                        padding=0,
                        margin=0,
                        content=ft.Icon(name=ft.icons.ARROW_FORWARD, size=25),
                        alignment=ft.alignment.center,
                    ),
                ],
            ),
    ...
    # Função para inserir dado_usuario no início da lista
    # @preparativos_insercao
    def clk_inserir_final_d(self, e):
        dado_usuario = self.ttf_novo_valor.value  # Obter dado do usuário
        # Cria um novo nó para ser adicionado na lista
        novo_noh = self.criar_noh(valor=dado_usuario)
        self.row_lista.controls.append(novo_noh)
        # self.row_lista.update()
        self.page.update()
